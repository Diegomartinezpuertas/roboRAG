...
- **And the comparison the review never asked for (ADR-033):** RAG against an
  LLM writing SQL over the same places, one session. The two tied on direct
  lookups (main 21/21 both, phrasing 18/18 both); on the spatial tasks RAG
  recovered the right position 3/6 and SQL 0/6 (hard 27/30 vs 24/30). A small
  experiment, presented as such.
